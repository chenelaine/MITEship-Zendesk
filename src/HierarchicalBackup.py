...
if not os.path.exists(backup_path):
  ...
  os.makedirs(backup_path + '\\categories')
...
print('Press any key to start the backup...')
input()
...
while endpoint:
     ...
     for article in data['articles']:
         ...
         categoryTitle = '<h1>CATEGORY: ' + categoryName + '</h1>\n'                
         sectionTitle = '<h2>SECTION:' + sectionName + '</h2>\n'         
         articleTitle = '<h3>ARTICLE:' + article['title'] + '</h3>'
         ...
         log.append((articleFilename, articleName, article['author_id']))
